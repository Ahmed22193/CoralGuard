...
export const AcceptDoctor = async(req ,res , next)=>{
    ...
    if(!doctor) return next(new Error("Doctor Not Found or he is accepted already",{cause:404}));
    SUCCESS(res , 200 ,"Doctor Accepted",doctor);
}
export const doctorsRegistration = async(req ,res , next)=>{
    const doctors = await UserModel.find({ userType: 'DOCTOR', acceptTerms: false });
    if(doctors.length === 0) return next(new Error("No Doctors Found to accepte",{cause:404}));
...
export const deleteConsultation = async(req, res, next) => {
    ...
    if(!consultation) return next(new Error("Consultation Not Found",{cause:404}));
...
export const getStats = async (req, res, next) => {
    ...
    const acceptedDoctorsCount = await UserModel.countDocuments({ acceptTerms: true });
    const unacceptedDoctorsCount = await UserModel.countDocuments({ acceptTerms: false });

    const consultationsWaiting = await ConsultationModel.countDocuments({ status: 'PENDING' });
    const consultationsDone = await ConsultationModel.countDocuments({ status: 'COMPLETED' });
    const consultationsPaid = await ConsultationModel.countDocuments({ status: 'PAID' });
    const consultationsAccepted = await ConsultationModel.countDocuments({ status: 'ACCEPTED' });
    const consultationsRejected = await ConsultationModel.countDocuments({ status: 'REJECTED' });
...
export const getAllDoctors = async (req, res, next) => {
    ...
    if (!doctors || doctors.length === 0) return next(new Error("No Doctors Found", { cause: 404 }));
    SUCCESS(res, 200, "All Doctors", doctors);
};
export const getAllPatients = async (req, res, next) => {
    const patients = await UserModel.find({ userType: 'PATIENT' });
    if (!patients || patients.length === 0) return next(new Error("No Patients Found", { cause: 404 }));
