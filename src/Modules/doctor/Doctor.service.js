...
export const Doctors = async(req , res ,next)=>{
    ...
    const doctors = await UserModel.find(filter).select("-password -role -__v -phone -createdAt -updatedAt");

    if(doctors.length === 0) return next(new Error("Not Found any Doctors", { cause: 404 }));
...
export const ConsultationsOrders = async (req, res, next) => {
  const { _id } = req.user;
  const consultations = await Consultations.find({ doctor: _id })
...
export const Report = async (req, res, next) => {
  try {
    ...
    if (!reportText) return next(new Error("من فضلك أدخل التقرير", { cause: 400 }));
    const updatedConsultation = await Consultations.findOneAndUpdate(
    ...
    if (!updatedConsultation) {
      return next(new Error(" الاستشارة غير موجودة أو لا تخصك او لم يتم الدفع", { cause: 401 }));
    }
    SUCCESS(res, 200, "تم تحديث حالة الاستشارة", updatedConsultation);
  } catch (err) {
    next(err);
